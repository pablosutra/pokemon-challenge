...
export const Menu = () => {
  ...
  useEffect(() => {
    axios
      .get(`${BASE_API_URL}${ENDPOINTS.list}`)
  ...
  return (
    <Fragment>
      {!isOpen && (
        <button className="menu-button" aria-expanded={isOpen} onClick={() => setIsOpen(true)}>
          ...
          {pokemonList.length &&
            pokemonList.map(({ name }, idx) => (
              <div className="burger-menu-list-item" key={idx}>
                ...
                <button className="burger-menu-list-item-name" onClick={() => _openPokemon(name)}>
                  {name}
                </button>
              </div>
            ))}
